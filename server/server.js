...
import { generateWorkoutPlan } from "../ai/ai.js";
...
app.post('/users/register', (req, res) => {
    const { email, password, user_id, session_id } = req.body;
    ...
    db.query(
        ...
        (err, results) => {
            ...
            db.query(
                'UPDATE onboarding_sessions SET user_id = ? WHERE session_id = ?',
                [user_id, session_id],
                (err, results) => {
                    if (err) return res.status(500).json({ error: err.message });
                    res.json({
                        message: 'User registered and session linked',
                        user_id: user_id,
